...
<section class="checkout-orders">
   ...
   <?php
      ...
      if($select_cart->rowCount() > 0){
         while($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)){
            $cart_items[] = $fetch_cart['name'].' (Rp'.number_format($fetch_cart['price'], 3, ',', '.').' x '. $fetch_cart['quantity'].') - ';
            $total_products = implode($cart_items);
            $grand_total += ($fetch_cart['price'] * $fetch_cart['quantity']);
   ?>
      <p> <?= $fetch_cart['name']; ?> 
         <span>(Rp<?= number_format($fetch_cart['price'], 3, ',', '.'); ?> x <?= $fetch_cart['quantity']; ?>)</span> 
      </p>
   <?php
         }
      }else{
   ...
   <div class="grand-total">Hasil Akhir :<span>Rp<?= number_format($grand_total, 3, ',', '.'); ?></span></div>
</div>
